...
async function fetchCollection(userId, type, subjectType = 2) {
  ...
  while (hasMore) {
    const url =
      `${API_BASE}/v0/users/${userId}/collections?subject_type=${subjectType}&type=${type}&limit=${limit}&offset=${offset}`;
    try {
...
async function processBookData(items, status) {
  ...
  for (const item of items) {
    ...
    const author = getFieldFromInfobox(subjectDetail?.infobox, [
      "作者",
      "作 者",
      "Author",
    ])
      || item.subject?.author
      || "";
    const publisher = getFieldFromInfobox(subjectDetail?.infobox, [
      "出版社",
      "出版社/出品",
      "Publisher",
    ]) || "";
...
async function processGameData(items, status) {
  ...
  for (const item of items) {
    ...
    const developer = getFieldFromInfobox(subjectDetail?.infobox, [
      "开发",
      "开发商",
      "Developer",
    ]) || "";
    const platform = getFieldFromInfobox(subjectDetail?.infobox, ["平台", "Platform"])
